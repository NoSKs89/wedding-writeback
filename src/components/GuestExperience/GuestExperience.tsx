...
const GuestExperience: React.FC<GuestExperienceProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const preRenderScrapbookImages = async () => {
      console.log('[SCRAPBOOK PRE-RENDER] Starting pre-render of scrapbook images...');
      ...
      const preRenderPromises = imageSources.slice(0, 15).map((src: string, index: number) => {
        return new Promise<void>((resolve) => {
          ...
          img.onerror = () => {
            console.warn('[SCRAPBOOK PRE-RENDER] Failed to pre-render:', src);
            resolve();
          };
      ...
      try {
        ...
        console.log('[SCRAPBOOK PRE-RENDER] Completed pre-rendering scrapbook images');
        ...
        console.error('[SCRAPBOOK PRE-RENDER] Error during pre-rendering:', error);
        setScrapbookImagesPreRendered(true);
      }
    };
